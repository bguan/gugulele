...
OVAL_LEN_RATIO = SNDHOLE_STYLE >= 8 ? [.3, .3, .333, .4, .4, .4][MODEL] :
				 SNDHOLE_STYLE >= 6 ? .25 : 0; // of body_rad
HOOK_WTH_RATIO = [.16, .14, .12, .1, .1, .11][MODEL]; // of body_rad
HOOK_LEN_RATIO = [.45, .48, .55, .52, .5, .45][MODEL]; // of body_rad
OVAL_WTH_RATIO = SNDHOLE_STYLE >= 8 ? [.09, .119, .13, .13, .13, .128][MODEL]: // of body_rad
                 SNDHOLE_STYLE >= 6 ? [.063, .071, .0841, .0857, .0645, .065][MODEL] : 0; 
OVAL_PLCMT_RATIO = [.4, .4, .4, .4, .4, .6][MODEL]; // plcmt of oval hole(s)
OVAL_WIDEN_RATIO = [.65, .65, .65, .65, .65, .65][MODEL]; // widen gap of oval holes(s)
...
// Bridge specs
BRDG_TCK = [4.5, 5, 5.5, 5.5, 5.5, 6][MODEL];
SDDL_RAD = 1;
...
TUNER_TOP_RAD = [7.5, 8.5, 5.5, 8, 8][TUNER_STYLE] + TOP_RND_RAD; 
TUNER_BOT_RAD = [5, 10, 11, 8.5, 8][TUNER_STYLE] + BOT_RND_RAD; 
...
ANCHORPIN_RAD =  [0, 1, 0, .45, .4][TUNER_STYLE]; 
ANCHORPIN_OFFSET = [0, 11, 0, 7.5, 7][TUNER_STYLE]; 
ANCHORPIN_DEP = [0, 8, 0, 3.5, 3][TUNER_STYLE];
STR_HOLE_FROM_COUNTER = [15, 22.5, 15, 23, 17.5][TUNER_STYLE];
ANCHORPIN_ANGLE = [45, 45, 0][HEAD_STYLE]; //45;
...
SPINE_PRE_LEN = [0, 1, H_GAP + N_GAP + S_GAP > 0 ? -5 : HEAD_STYLE == 0 ?-15 : -25, 10][SPINE_STYLE];
SPINE_LEN = 
    SPINE_STYLE == 0 ? 0 :
