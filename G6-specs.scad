...
SHOULDER_FLARE = [99, 101, 102, 102.6, 103, 104][MODEL]; 
...
// Bridge specs
BRDG_TCK = 1+SCALE_LEN/80; //[4.5, 5, 5.5, 5.5, 5.5, 6][MODEL];
SDDL_RAD = 1;
BRDG_WTH = (HEAD_STYLE==1 && STRTIE_STYLE==1 ? 1.2 : 1)*SCALE_LEN/30; //SCALE_LEN/25;
BRDG_CARVE_SCALE = .375; //[.6, .425, .375, .325, .275, .225][MODEL];
...
// Shoulder Specs
SHOULDER_SPLIT_RATIO = .42; // only when S_GAP > 0 to determine front vs back
SHOULDER_JNT_LEN = .05*SCALE_LEN; 
SHOULDER_JNT_WTH = NUM_STRS*NUT_HOLE_GAP +4;
...
BRACE_X_WIDEN_RATIO = (SNDHOLE_STYLE < 8 ? .5 : .6); // wide gap as ratio of body rad
...
SHOULDER_SCREW_MDL = V_GAP > 0 ? NECK_SCREW_MDL : "M1.6x20";
...
SHOULDER_SCREW_HEAD_RAD = NECK_SCREW_HEAD_RAD; 
SHOULDER_SCREW_HEAD_TCK = NECK_SCREW_HEAD_TCK; 
...
HEADLESS_STRING_ANGLE = [ 
    [40, 44, 44, 44, 44, 49][MODEL], 0, 
    [28, 29, 28, 27, 26, 29][MODEL] +(HEAD_POKED ?1 :0) 
...
HEADLESS_FRONT_GROOVE_RAD = [ 
    (HEAD_POKED ? SPINE_RAD+1 : 2*STR_HOLE_RAD),
    0,
...
CAVITY_DOME_SIDE_STRETCH = NUM_STRS/3.2;
...
TUNER_FANOUT_RAD = butt_len(NECK_LEN, NECK_HEAD_WTH, NECK_SLOPE, SCALE_LEN, 
                    SHOULDER_FLARE, FRONT_BACK_RATIO) -1 
				-max(TUNER_TOP_RAD, TUNER_BOT_RAD) -max(TOP_RND_RAD, BOT_RND_RAD);
STR_GUIDE_PLCMT = SCALE_LEN + max(TUNER_CAVITY_DEP, TUNER_FANOUT_RAD -(NUM_STRS/1.5)*TUNER_TOP_RAD);
